...
//            String imgStr = Base64Util.encode(imgData);
            ...
            if (display_order == -1) {
                imagesList = imagesDao.findAllByGoodsId(goods_id);
            } else {
                Images image = imagesDao.findByGoodsIdAndAndDisplayOrder(goods_id, display_order);
                if (image != null) imagesList.add(image);
            }
            for (Images images : imagesList) imgStrList.add(images.getImg());
            /* 获得答案 */
            List<String> resultList = new ArrayList<>();
