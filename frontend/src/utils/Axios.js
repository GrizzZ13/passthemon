...
async function getLocalToken() {
  let val = await storage.load({key: 'loginState'});
  console.log('getLocalToken' + val.token);
  return val.token;
}
...
instance.interceptors.response.use(
  response => {
    ...
    if (msg.status === UPDATE_TOKEN) {
      ...
      NavigationService.navigate('Error',{type:PERMISSION_DENIED});
      return Promise.reject(response);
    } else if (msg.status === VISITING_TOO_OFTEN){
      NavigationService.navigate('Error',{type:VISITING_TOO_OFTEN});
      return Promise.reject(response);
    }else if(msg.status === REQUEST_EXCESSIVE){
      NavigationService.navigate('Error',{type:REQUEST_EXCESSIVE});
      return Promise.reject(response);
    }
