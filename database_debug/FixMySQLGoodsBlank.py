...
def fixMySQLNames():
    ...
    search_sql = 'SELECT name FROM goods WHERE goods_id = %s'
    update_sql = 'UPDATE goods SET name = %s WHERE goods_id = %s'
    logging.info('Start change names...')
    for goods_id in range(min_goods_id, max_goods_id + 1):
        cursor.execute(search_sql, goods_id)
        name = cursor.fetchone()[0]
        name = str(name).lstrip()
        cursor.execute(update_sql, [name, goods_id])
        connection.commit()
